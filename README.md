...
* **UX（ユーザー体験）設計**: キーボードショートカット（Enterキー）や、業務フローに合わせたボタン配置の実装
...
* **2026/05/04**: **UXの最適化（Enterキー対応、予約→利用中への直接遷移）を実装。実用性を追求したアップデートを完了** ✅ [NEW]
...
* **フルクラウド同期**: すべての席情報（空席・予約・利用中）をFirestoreで一元管理。店側が操作した瞬間に客側の画面も**リロードなしで即座に**変化する双方向同期。
...
* **バリデーション強化**: 重複する席名の登録防止や、不正なステータス変更のガード。
